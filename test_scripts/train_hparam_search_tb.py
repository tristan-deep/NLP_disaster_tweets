...
if __name__ == '__main__' :
    ...
    HP_MODEL_NAME = hp.HParam('model_name', hp.Discrete(['LSTM_CNN_deep_model']))
    HP_DROPOUT = hp.HParam('dropout', hp.Discrete([0.5]))
    HP_OPTIMIZER = hp.HParam('optimizer', hp.Discrete(['adam']))
    HP_LSTM_OUT = hp.HParam('lstm_output_size', hp.Discrete([20]))
    HP_CONV_MODULES = hp.HParam('conv_num_modules', hp.Discrete([1]))
    HP_CONV_FILTERS = hp.HParam('conv_num_filters', hp.Discrete([16]))
    HP_CONV_FILTER_SIZE = hp.HParam('conv_filter_size', hp.Discrete([1]))
                    ...
                    for conv_num_modules in HP_CONV_MODULES.domain.values:
                        for conv_num_filters in HP_CONV_FILTERS.domain.values:
                            for conv_filter_size in HP_CONV_FILTER_SIZE.domain.values:
                                ...
                                #          callbacks= callbacks)
                                plot_model(model, to_file="images/model_summary/"+hparams[HP_MODEL_NAME]+".png", show_shapes = True)
                                if save_model == True:
